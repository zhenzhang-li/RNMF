...
samPerMutSigs <- function(Sfile = NULL, choose.Sigs = 'SBS1', file = NULL, sample.id = 'Tumor_Sample_Barcode', Hugo = 'Hugo_Symbol', Mutfreq = 0.04, threshold = 0.06, Qvalue = 0.1, plot = TRUE) 
{
  if(is.null(Sfile))
  {
  ...
  dir.create('samplesResults/samPerMutSigs/', showWarnings = FALSE, recursive = TRUE, mode = "0777")
  ...
  write.table(Gmatrix, file=paste('samplesResults/samPerMutSigs/Sam.final.mutation_signature.',choose.Sigs,'.xls',sep=""),quote=F,col.names=T,row.names=F,sep="\t")
    
  if(plot)
  {
    file2 = paste('samplesResults/samPerMutSigs/Sam.final.mutation_signature.',choose.Sigs,'.xls',sep="")
    ...
    pdf(paste('samplesResults/samPerMutSigs/Sam.final.mutation_signature.',choose.Sigs,'.pdf',sep=""),6,6)
    plot(0,0,type='l',xlim=c(0,1),ylim=c(0,ceiling(max(y)+1)),xlab='Median mutational exprosure of samples with non-silent mutations',ylab="-log10( PerMutation P value )")
    n=length(y)
    col=rep(NA,n)
    for(i in 1:n){
